...
* `POST /api/event/accept` - принять приглашение на событие

    Тело запроса:
    ```
    {
        "event_id": "<уникальный id ивента>",
        "is_regular": (true|false) // если принимается приглашение на регулярную встречу, то is_regular = true
    }
    ```
